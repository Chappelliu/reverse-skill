...
**lint 执行范围（重要 —— 决定哪些漏标会被拒收）**：

| 层级 | lint 是否强制 | 说明 |
|------|:---:|------|
| H2（战术级） | ✅ 强制 | `lint-attack-tags.ps1` 检查所有 H2，战术相关标题无 `(Txxxx)` 即报错。 |
| H3（技术级） | ❌ 不强制（advisory） | H3 数量远多于 H2，全量强制会产生大量噪音淹没真问题。H3 标注作为**渐进式 roll-out**：新写文档 SHOULD 标，旧文档按需补，lint 不阻断。 |
| H4+ | ❌ 不检查 | 太细，不值得。 |

**写 H3 时的锚点注意**：GitHub 生成 anchor 时会把 `/` `(` `)` `&` `·` `（）` 等标点**全部删除**，并把连续空白折叠为**单个** `-`。因此 `### 凭证获取 / Credential Access (TA0006)` 的 anchor 是 `#凭证获取-credential-access-ta0006`（**单**连字符，不是 `--`）。引用 H3 章节时用 `verify-md-anchors.ps1` 校验。
